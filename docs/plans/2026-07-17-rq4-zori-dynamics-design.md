...
| 5 | Flow | New `src/pipelines/panel.py::build_panel_flow`, separate from `build_metro_flow`; reuses cached fetch tasks; `run_pipeline.py --panel`; `--verify` pairs manifests via `output_csv` (owned edit); dev-cache salt note |
